...
The following roles are available. Each has skill areas organized by career level (Level 1: Early Career, Level 2: Mid Career) that the learner needs to master:
...
3. **Skill self-assessment** (for role path) — Once a role is identified, show the Level 1 (Early Career) skill areas as multi-select pills and ask: "Which of these skills do you already feel confident in?" Skills they DON'T select become "should learn" priorities.

4. **Timeline** — Duration and weekly hours (e.g., "3-6 months at 6 hours/week"). Always ask — do not assume.

## Skill Gap Categorization

Based on the learner's current background and target role, categorize each skill area:
- **should**: Level 1 skill areas the learner does NOT have that are essential for the target role. These are the primary gaps.
- **might**: Level 2 skill areas or Level 1 areas the learner has some background in but needs to deepen.
- **optional**: Nice-to-have skill areas that complement the role but aren't critical gaps.

If the learner provides no current role/background, default all Level 1 skills to "should" and Level 2 skills split between "might" and "optional".
...
Structure milestones around skill mastery, not course sequences:
- Each milestone targets 1-3 specific skill areas from the role catalog
- "Should" skills get dedicated milestones first
- "Might" skills follow, then "optional"
- Milestone names: "Phase N: Master [Skill X & Skill Y] (duration)"
- Milestone descriptions: "Goal: skill1, skill2, skill3"
...
- Match courses to the milestone's target skills
...
- Each course's targetSkillIds should list which skill areas it advances
...
- "should" skills: xpTarget of 375-750 (enough to reach Developing or Comprehending)
- "might" skills: xpTarget of 250-375
- "optional" skills: xpTarget of 125-250
- Total XP across all milestones for a "should" skill should aim toward the skill's xpMax (expressions × 300 XP each)
...
- title, role, skills (exactly 3 top-level skills)
- skillBreakdown: { should: [...], might: [...], optional: [...] } with skill NAMES
- totalDuration, hoursPerWeek
- milestones array with targetSkills per milestone
